/**
 * Simplified test script for AWS Fargate with ALB
 * 
 * Usage:
 *   npx ts-node -P scripts/tsconfig.json scripts/test-simple-alb.ts
 */
...
async function main() {
  // Configuration
  const serverName = `test-simple-alb-${Date.now().toString().slice(-6)}`;
  
  console.log(`\n=== Testing Simple ALB Launch ===`);
  console.log(`Server Name: ${serverName}\n`);
  
  try {
    // Launch container
    console.log('Launching container with ALB...');
    const result = await launchContainer({
      image: 'ghcr.io/github/mcp-server', // Use GitHub's official MCP server
      serverName,
      containerPort: 8000,     // MCP servers typically use port 8000
    });
    ...
    if (connected) {
      console.log(`\nEndpoint is now accessible! ALB is working correctly.`);
    } else {
      console.log(`\nCouldn't connect after ${maxAttempts} attempts. The ALB might still be initializing.`);
    }
